defmodule Bench do
  ...
  def bench(i, n) do
    ...
    list = Enum.reduce(seq,  SeemaList.new(),  fn(e, list) -> SeemaList.add(list, e, :foo) end)
    tree = Enum.reduce(seq,  SeemaTree.new(),  fn(e, tree) -> SeemaTree.add(tree, e, :foo) end)
    map = Enum.reduce(seq,  Map.new(),  fn(e, map) -> Map.put(map, e, :foo)  end)

    seq = Enum.map(1..n, fn(_) -> :rand.uniform(i) end)

    {tla, _} = :timer.tc(fn() -> Enum.each(seq, fn(e) -> SeemaList.add(list, e, :foo) end) end)
    {tta, _} = :timer.tc(fn() -> Enum.each(seq, fn(e) -> SeemaTree.add(tree, e, :foo) end) end)
    {tma, _} = :timer.tc(fn() -> Enum.each(seq, fn(e) -> Map.put(map, e, :foo) end) end)

    {tll, _} = :timer.tc(fn() -> Enum.each(seq, fn(e) -> SeemaList.lookup(list, e) end) end)
    {ttl, _} = :timer.tc(fn() -> Enum.each(seq, fn(e) -> SeemaTree.lookup(tree, e) end) end)
    {tml, _} = :timer.tc(fn() -> Enum.each(seq, fn(e) -> Map.get(map, e) end) end)

    {tlr, _} = :timer.tc(fn() -> Enum.each(seq, fn(e) -> SeemaList.remove(list, e) end) end)
    {ttr, _} = :timer.tc(fn() -> Enum.each(seq, fn(e) -> SeemaTree.remove(tree, e) end) end)
